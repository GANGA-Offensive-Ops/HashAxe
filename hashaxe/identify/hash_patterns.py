...
_PATTERNS: list[tuple[re.Pattern, str, str, float]] = [
    ...
    ), "hash.argon2", "Argon2", 1.0),

    # ── scrypt ────────────────────────────────────────────────────────────────
    # ── scrypt ────────────────────────────────────────────────────────────────
    (re.compile(
        r'^\$scrypt\$ln=(\d+),r=(\d+),p=(\d+)\$([A-Za-z0-9+/]+)\$([A-Za-z0-9+/]+)$|'
        r'^SCRYPT:(\d+):(\d+):(\d+):([A-Za-z0-9+/=]+):([A-Za-z0-9+/=]+)$', re.IGNORECASE
    ), "hash.scrypt", "scrypt", 1.0),

    # ── bcrypt ────────────────────────────────────────────────────────────────
    (re.compile(
        r'^\$2([aby])\$(\d{2})\$([./A-Za-z0-9]{53})$'
    ), "hash.bcrypt", "bcrypt", 1.0),

    # ── Unix crypt: sha512crypt ───────────────────────────────────────────────
    (re.compile(
        r'^\$6\$(rounds=(\d+)\$)?([./A-Za-z0-9]+)\$([./A-Za-z0-9]{86})$'
    ), "hash.sha512crypt", "sha512crypt $6$", 1.0),

    # ── Unix crypt: sha256crypt ───────────────────────────────────────────────
    (re.compile(
        r'^\$5\$(rounds=(\d+)\$)?([./A-Za-z0-9]+)\$([./A-Za-z0-9]{43})$'
    ), "hash.sha256crypt", "sha256crypt $5$", 1.0),

    # ── Unix crypt: md5crypt ──────────────────────────────────────────────────
    (re.compile(
        r'^\$1\$([./A-Za-z0-9]{1,8})\$([./A-Za-z0-9]{22})$'
    ), "hash.md5crypt", "md5crypt $1$", 1.0),

    # ── Unix crypt: DES (legacy) ──────────────────────────────────────────────
    (re.compile(
        r'^[./A-Za-z0-9]{13}$'
    ), "hash.descrypt", "DES crypt", 0.4),

    # ── MySQL native password (double SHA-1) ──────────────────────────────────
    (re.compile(
        r'^\*[A-Fa-f0-9]{40}$'
    ), "hash.mysql", "MySQL native_password", 0.95),

    # ── MSSQL ─────────────────────────────────────────────────────────────────
    (re.compile(
        r'^0x0100[A-Fa-f0-9]{88}$'
    ), "hash.mssql2005", "MSSQL 2005+", 0.9),
    (re.compile(
        r'^0x0200[A-Fa-f0-9]{136}$'
    ), "hash.mssql2012", "MSSQL 2012+", 0.9),

    # ── JWT (HMAC) ────────────────────────────────────────────────────────────
    (re.compile(
        r'^eyJ[A-Za-z0-9_-]+\.eyJ[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+$'
    ), "hash.jwt", "JWT (HMAC)", 0.95),
    ...
    ), "network.ntlmv2", "NetNTLMv2", 1.0),

    (re.compile(
        r'^([^:]+)::([^:]*):([a-fA-F0-9]{48}):([a-fA-F0-9]{16})$'
    ), "network.ntlmv1", "NetNTLMv1", 0.95),

    # ── Kerberos family ───────────────────────────────────────────────────────
    # Kerberoast TGS-REP RC4 etype 23 (hashcat -m 13100)
    (re.compile(
        r'^\$krb5tgs\$23\$\*[^$]+\$[^$]+\$[^$*]+\*\$[a-fA-F0-9]{32}\$[a-fA-F0-9]+$'
    ), "network.krb5tgs_rc4", "Kerberoast TGS RC4", 1.0),

    # AS-REP Roast etype 23 (hashcat -m 18200)
    (re.compile(
        r'^\$krb5asrep\$23\$[^:@]+@?[^:]*:[a-fA-F0-9]{32}\$[a-fA-F0-9]+$'
    ), "network.krb5asrep_rc4", "AS-REP Roast RC4", 1.0),

    # Kerberos TGS AES128 etype 17 (hashcat -m 19600)
    (re.compile(
        r'^\$krb5tgs\$17\$[^$]+\$[^$]+\$\*[^$*]*\*\$[a-fA-F0-9]+\$[a-fA-F0-9]+$'
    ), "network.krb5tgs_aes128", "Kerberos TGS AES128", 1.0),

    # Kerberos TGS AES256 etype 18 (hashcat -m 19700)
    (re.compile(
        r'^\$krb5tgs\$18\$[^$]+\$[^$]+\$\*[^$*]*\*\$[a-fA-F0-9]+\$[a-fA-F0-9]+$'
    ), "network.krb5tgs_aes256", "Kerberos TGS AES256", 1.0),

    # ── Domain Cached Credentials ────────────────────────────────────────────
    # DCC2 MS Cache v2 (hashcat -m 2100) — must come before DCC1
    (re.compile(
        r'^\$DCC2\$\d+#[^#]+#[a-fA-F0-9]{32}$'
    ), "network.dcc2", "DCC2 MS Cache v2", 1.0),

    # ── PostgreSQL MD5 (only matches explicit md5 prefix formats) ──────────────
    (re.compile(
        r'^md5[a-f0-9]{32}$|^[a-zA-Z0-9._-]+:md5[a-fA-F0-9]{32}$', re.IGNORECASE
    ), "hash.postgres", "PostgreSQL MD5", 0.95),

    # DCC v1 (hashcat -m 1100): hash:username
    (re.compile(
        r'^[a-fA-F0-9]{32}:[a-zA-Z0-9._@\-]+$'
    ), "network.dcc1", "DCC MS Cache v1", 0.85),

    # ── DPAPI ────────────────────────────────────────────────────────────────
    (re.compile(
        r'^\$DPAPImk\$\d\*\d\*'
    ), "disk.dpapi", "DPAPI Masterkey", 1.0),
    ...
    (re.compile(
        r'^[a-fA-F0-9]{128}$'
    ), "hash.sha512", "SHA-512", 0.7),

    # SHA-384 — 96 hex chars
    (re.compile(
        r'^[a-fA-F0-9]{96}$'
    ), "hash.sha384", "SHA-384", 0.65),

    # SHA-256 — 64 hex chars
    (re.compile(
        r'^[a-fA-F0-9]{64}$'
    ), "hash.sha256", "SHA-256", 0.7),

    # SHA-224 — 56 hex chars
    (re.compile(
        r'^[a-fA-F0-9]{56}$'
    ), "hash.sha224", "SHA-224", 0.6),
...
SUPPORTED_HASH_FORMATS: dict[str, str] = {
    entry[1]: entry[2] for entry in _PATTERNS
}
